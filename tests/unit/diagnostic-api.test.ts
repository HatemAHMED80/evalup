...
// Requiert le serveur dev en marche sur localhost:3000

const BASE_URL = process.env.TEST_BASE_URL || 'http://localhost:3000'

// -----------------------------------------------------------------------------
// Mini test runner (async)
// -----------------------------------------------------------------------------
...
async function main() {
  ...
  await test('Corps vide → 400', async () => {
    const res = await fetch(`${BASE_URL}/api/diagnostic`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: '{}',
  ...
  await test('JSON invalide → 400', async () => {
    const res = await fetch(`${BASE_URL}/api/diagnostic`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: 'not json',
  ...
  await test('Réponse valide contient archetypeId, archetype, multiples, input', async () => {
  ...
  await test('Input echo contient les champs envoyés', async () => {
  ...
  await test('Archetype retourné a les champs requis', async () => {
    const { data } = await postDiagnostic({
      activityType: 'saas',
      revenue: 5000000,
      ebitda: 1200000,
  ...
  await test('hasPatrimoine default à false si non fourni', async () => {
    const { data } = await postDiagnostic({
      activityType: 'saas',
      revenue: 5000000,
      ebitda: 1200000,
  ...
  await test('loyersNets default à null si non fourni', async () => {
    const { data } = await postDiagnostic({
      activityType: 'saas',
      revenue: 5000000,
      ebitda: 1200000,
  ...
  const DETECTION_CASES: DetectionTestCase[] = [
    ...
    },
    {
      ...
      body: { activityType: 'marketplace', revenue: 20000000, ebitda: 2000000, growth: 30, recurring: 20, masseSalariale: 30, effectif: '50+' },
    ...
    {
      name: 'Industrie (manufacturing) → industrie',
      body: { activityType: 'manufacturing', revenue: 8000000, ebitda: 900000, growth: 3, recurring: 10, masseSalariale: 40, effectif: '50+' },
      expected: 'industrie',
    },
    {
  ...
  await test('activityType=saas → hasMRR=true automatique (detect saas_mature)', async () => {
    const { data } = await postDiagnostic({
      activityType: 'saas',
      revenue: 5000000,
      ebitda: 1200000,
  ...
  await test('activityType=commerce → hasPhysicalStore=true automatique (detect commerce_retail)', async () => {
    const { data } = await postDiagnostic({
      activityType: 'commerce',
      revenue: 800000,
      ebitda: 120000,
  ...
  await test('activityType=conseil → ni hasMRR ni hasPhysicalStore', async () => {
    const { data } = await postDiagnostic({
  ...
  await test('SaaS mature retourne des multiples non-null', async () => {
    const { data } = await postDiagnostic({
      activityType: 'saas',
      revenue: 5000000,
      ebitda: 1200000,
  ...
  await test('Commerce retourne des multiples non-null', async () => {
    const { data } = await postDiagnostic({
      activityType: 'commerce',
      revenue: 800000,
      ebitda: 120000,
  ...
  await test('growth/recurring/masseSalariale default à 0 si absents', async () => {
    const { status, data } = await postDiagnostic({
  ...
  await test('nafCode optionnel accepté', async () => {
