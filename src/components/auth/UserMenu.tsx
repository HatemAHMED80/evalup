  ...
  return (
    <div className="relative" ref={menuRef}>
      <button
        ...
        className="w-full flex items-center gap-3 p-1.5 rounded-[var(--radius-md)] hover:bg-[var(--bg-tertiary)] transition-colors"
      >
        {user ? (
          // Utilisateur connecte - afficher avatar avec initiales
          <div className="w-9 h-9 bg-[var(--accent)] rounded-full flex items-center justify-center flex-shrink-0">
        ...
        <div className="flex-1 min-w-0 text-left">
          ...
          <div className={`text-xs ${isPremium ? 'text-[var(--accent)]' : 'text-[var(--text-tertiary)]'}`}>
            {isPremium ? `⭐ ${planName}` : 'Plan Gratuit'}
          </div>
        </div>
        <svg
          className={`w-4 h-4 text-[var(--text-tertiary)] transition-transform flex-shrink-0 ${isOpen ? 'rotate-180' : ''}`}
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
      ...
      {isOpen && (
        <div className="absolute left-0 bottom-full mb-2 w-56 bg-[var(--bg-secondary)] border border-[var(--border)] rounded-[var(--radius-lg)] shadow-[var(--shadow-lg)] py-1 z-50">
          ...
          {!user && (
            <div className="py-1">
              <Link
                ...
                className="flex items-center gap-3 px-4 py-2 text-sm text-[var(--accent)] hover:bg-[var(--accent-light)] transition-colors"
              >
          ...
          {user && (
            <div className="border-t border-[var(--border)] py-1">
              <button
                ...
                className="flex items-center gap-3 w-full px-4 py-2 text-sm text-[var(--danger)] hover:bg-[var(--danger)]/10 transition-colors"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
