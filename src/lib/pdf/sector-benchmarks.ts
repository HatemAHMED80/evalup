...
export const BENCHMARKS: Record<string, SectorBenchmarks> = {
  ...
  commerce_gros: {
    nom: 'Commerce de Gros / Distribution',
    margeNette: { min: 0.005, median: 0.02, max: 0.04 },
    margeEbitda: { min: 0.02, median: 0.05, max: 0.08 },
    croissanceCA: { min: -0.03, median: 0.03, max: 0.08 },
    dso: { min: 30, median: 50, max: 70 },
    multipleCA: { min: 0.2, max: 0.6 },
    multipleEbitda: { min: 3, max: 4 },
  },
  ecommerce: {
    nom: 'E-commerce / D2C',
...
export function getSectorFromNaf(codeNaf: string): string {
  ...
  const mapping: Record<string, string> = {
    ...
    '45': 'commerce', '46': 'commerce_gros', '47': 'commerce',
